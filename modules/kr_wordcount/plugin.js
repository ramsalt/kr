        ...
        if ($('div#cke_wordcount_'+editor.name).length > 0) { // Check element exists
            ...
            setTimeout(function() {
                ...
                if (editor.config.wordcount_maxWords > 0) {
                    // Display with limit
                    $('div#cke_wordcount_'+editor.name).html('Ord: '+wordCount+'/'+editor.config.wordcount_maxWords+' Tegn: '+ GetCharCount(editor.getData()));
                } else {
                    // Just display word count

                    // char_count=GetCharCount(content);
                    // $('#field-op-author-items').html(content+' '+char_count);
                    $('div#cke_wordcount_'+editor.name).html('Ord: '+wordCount+' Tegn:'+ GetCharCount(editor.getData())); 
                }
    ...
    CKEDITOR.plugins.add('wordcount', {
        init: function(editor) {
            ...
            setTimeout(function() {
                if (editor.config.wordcount_maxWords > 0) { 
                    // Display with limit
                    html = 'Ord: '+GetWordCount(editor.getData())+'/'+editor.config.wordcount_maxWords+ ' Tegn: '+ GetCharCount(editor.getData());
                    $('td#cke_bottom_'+editor.name).append('<div id="cke_wordcount_'+editor.name+'" style="display: inline-block; float: right; text-align: right; margin-top: 5px; cursor:auto; font:12px Arial,Helvetica,Tahoma,Verdana,Sans-Serif; height:auto; padding:0; text-align:left; text-decoration:none; vertical-align:baseline; white-space:nowrap; width:auto;">'+html+'</div>');
                } else {
                    // Just display word count
                    html = 'Ord: '+GetWordCount(editor.getData())+ ' Tegn: '+ GetCharCount(editor.getData());
                    $('td#cke_bottom_'+editor.name).append('<div id="cke_wordcount_'+editor.name+'" style="display: inline-block; float: right; text-align: right; margin-top: 5px; cursor:auto; font:12px Arial,Helvetica,Tahoma,Verdana,Sans-Serif; height:auto; padding:0; text-align:left; text-decoration:none; vertical-align:baseline; white-space:nowrap; width:auto;">'+html+'</div>');
                }
            }, 4000);
