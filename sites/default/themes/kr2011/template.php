...
function _kr_bildegalleri(){
  ...
  if($node->field_rel_bildegalleri[0]['nid']>1){
      drupal_add_js(drupal_get_path('theme', 'kr2011').'/flex/jquery.min.js');
      drupal_add_js(drupal_get_path('theme', 'kr2011').'/flex/jquery-noconflict.js');
      drupal_add_js(drupal_get_path('theme', 'kr2011').'/flex/jquery.flexslider.js', 'theme', 'header');
      drupal_add_js(drupal_get_path('theme', 'kr2011').'/flex/jquery.colorbox.js', 'theme', 'header');    
      ...
      foreach ($node->field_galleri_bilde as $delta => $item) {
        ...
        $html .= $item['view'];
        if($node->field_galleri_desc[$delta]['safe'] && $node->field_galleri_kredit[$delta]['safe']){
          $html .= '<p>'.$node->field_galleri_desc[$delta]['safe'];
          $html .= '<span class="kredit">'.$node->field_galleri_kredit[$delta]['safe'].'</span></p>';
        }elseif($node->field_galleri_desc[$delta]['safe']){
          $html .= '<p>'.$node->field_galleri_desc[$delta]['safe'].'</p>';
        }elseif ($node->field_galleri_kredit[$delta]['safe']) {
          $html .= '<p><span class="kredit">'.$node->field_galleri_kredit[$delta]['safe'].'</span></p>';
        }
        $html .='</a>';
        $html .='</li>';
        ...
        <div class="caption" style="width: '.$image['width'].'px;">'.$node->field_galleri_desc[$delta]['safe'].'</div>
        </div>';
      }
      $html .='</ul></div>';
      $hidden .='</div>';
